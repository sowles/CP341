...
void *qsend(void *args)
{
	...
	while (1)
	{
		...
		binaryLength = strlen(text) * 16; // each char is 16-bit binary
...
{ // char =ascii char, eight==char arr for binary string to be stored (9 long to hold 8 bit and terminator)
...
bool withinBuffer(uint32_t tick, int edgeDirection)
{
	...
	if ((tick < (correctTick + buff)) && (tick > (correctTick - buff)))
	{
		...
		if (bitCt % 16 == 0)
		{ // TODO change back to 16 instead of 8, as well as the next 4 lines
			// printf("Byte sent\n");
