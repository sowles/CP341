...
Queue q; //for queue
char* myLastMove = NULL;
bool firstMove = true;
//PROTOTYPEŠ
void* receiveHandshake(void* args);
void* qsend(void* args);
int rps_logic(Packet* opp_packet);
void rps_game(Packet* opp_packet, int headerType);
...
//	printf("chartobinary %s\n", etest);
	...
	printf("Enter RPS to start a rock, paper, scissors game. \n");
	printf("Or, send a chat message\n");
...
while(1){
		...
		if (txt[0]=='R' && txt[1] =='P' && txt[2] == 'S'){
			// initiator calls this and sends into FIR mode
			if (firstMove){
				rps_game(packet, FIR);
			}
			else{
...
void rps_game(Packet* packet, int headerType){
	// gets user input and enqueues a packet 
	// if FIR is called, it was the initiator
	// if SEC is called, it is the response	
		uint8_t sender_addy = YOUR_PI_ADDRESS;
		//prompt user for rock paper or scissors
		char* user_choice = NULL;
		size_t size =0;

		printf("Enter your MOVE: ROCK (r), PAPER (p), SCISSORS (s)): ");
		getline(&user_choice, &size, stdin); //TODO implement error checking here
		...
		// RPS --> I started the game
		// FIN --> opponent started game
		char* toSend = malloc(sizeof(char)*(strlen(user_choice)+3));
		if (headerType == FIR){
			toSend[0] = 'F';
			toSend[1] = 'I';
			toSend[2] = 'R';
			toSend[3] = user_choice[0];
			myLastMove = &toSend[3];
		}	
		else{
			// we are the responder, should finsih the game
			// Then send a packet with the results to be printed to initiator
			int result = rps_logic(opp_packet);
			// send result to initiator and print results - for responder 0-draw, 1-win, 2-loss
			printf("result: %d\n", result);
			toSend[0] = 'S';
			toSend[1] = 'E';
			toSend[2] = 'C';
	...
	//LOGIC: 
	//I start game -> call rps_game(..., RPS), I save my move in myLastMove
	//Sends msg with RPS in header followed by my move
	//Opp receives this, reads the RPS in the header, calls rps_game(..., FIN)
	//Opp sends msg with FIN in header followed by move
	//I receive msg with FIN in header, read the move and see who won

		// [1,1,1,1]
		packet ->sender_addy = sender_addy & 0x0F;
		packet ->receiver_addy = receiver_addy & 0x0F; // & 0x0F to mask and ensure 4 bits (decimal:0-15) [1,1,1,1]
		//packet -> msg_type = RPS_START; 
		packet ->data =strdup(toSend); // dupe string to allocate separate memory
		...
		free(packet->data);
		free(packet);
		//TODO implement logic so you can't start a game once you're in one
}


int rps_logic(Packet* opp_packet){

	// I started game, my move saved in myLastMove, opp is in opp_packet

	
	char opp = opp_packet->data[3];
	printf("opp %c, mine %s", opp, myLastMove);
	//TODO change this to call required funcs
...
void* receiveHandshake(void* args){
	Packet packet;
	callback(pi,rx,FALLING_EDGE,fallingFunc);
	callback(pi,rx,RISING_EDGE,risingFunc);
		...
		if ((lastFallTime != 0) && (diff > 40*DELTA_T)){
			...
			//	printf("TEST1");
				if (packet.receiver_addy == YOUR_PI_ADDRESS){
					// check for rps mode
					// RPS in beginning of msg => someone else started rps
					// FIN in beginning of msg => I started rps
					if (packet.data[0] == 'F' && packet.data[1] == 'I' && packet.data[2] == 'R'){
						firstMove = false;
						// get the move, store in myFirstMove
						//rps_game(packet.receiver_addy, NOT);	
					}
					else if (packet.data[0] == 'S' && packet.data[1] == 'E' && packet.data[2] == 'C'){
					// read this and print results	
						//printf("test: %s", myLastMove);
						// calculate winner
						rps_logic(&packet);
					}
